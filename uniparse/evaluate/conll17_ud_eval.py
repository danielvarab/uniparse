...
def evaluate_wrapper(gold_file, system_file, weights):
    # Load CoNLL-U files
    gold_ud = load_conllu_file(gold_file)
    system_ud = load_conllu_file(system_file)

    # Load weights if requested
    deprel_weights = load_deprel_weights(weights)

    return evaluate(gold_ud, system_ud, deprel_weights)

def main(verbose, weights, gold_file, system_file):
    # Use verbose if weights are supplied
    if weights is not None and not verbose:
        verbose = 1

    # Evaluate
    evaluation = evaluate_wrapper(gold_file, system_file, weights)
...
if __name__ == "__main__":
    ...
    main(options.verbose, options.weights, options.gold_file, options.system_file)
